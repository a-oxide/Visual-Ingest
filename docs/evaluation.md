...
node scripts/evaluate-paccar.mjs <path-to-your-pdf>
# Then read docs/eval-output.txt
```

The evaluation script takes the PDF path as a CLI argument and writes output to `docs/eval-output.txt` relative to the current working directory. `docs/eval-output.txt` is gitignored.

## Security note

`docs/eval-output.txt` (when generated by the evaluation script) contains the full transcription of the test document, which may be copyrighted material. It is **gitignored and not pushed** to the public GitHub repository. Only this evaluation.md summary is published.
