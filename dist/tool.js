...
const z = tool.schema;
export function buildToolResult(results, budget, docSlug, sourcePath) {
    const pageResults = results.map((r) => ({
        page: r.page,
        class: r.class,
...
export function makeTool(options) {
    ...
    return tool({
        description: "Read a PDF or image file by rendering each page to an image and processing it with a vision model. " +
