...
export function truncateToBudget(pages, budget, docSlug) {
    const indexLines = pages.map((p) => `page ${p.page}: ${p.class}, ${p.charCount} chars, ${p.visionStatus}`);
    const index = indexLines.join("\n");
    const totalTokens = estimateTokens(pages.map((p) => p.merged).join("\n\n"));
